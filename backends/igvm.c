...
static const struct IGVMDirectiveHandler directive_handlers[] = {
...
static int directive(QemuIgvm *ctx, uint32_t type, Error **errp)
{
    size_t handler;
    ...
    for (handler = 0; handler < G_N_ELEMENTS(directive_handlers); handler++) {
        ...
        result = directive_handlers[handler].handler(ctx, header_data, errp);
                                   ...
                                   Error **errp)
{
    ...
    /* If a confidential guest support object is provided then use it to set the guest state. */
    if (ctx->cgs) {
        cgs_page_type =
                                      ...
                                      Error **errp)
{
    ...
    QTAILQ_FOREACH(param_entry, &ctx->parameter_data, next)
    {
        if (param_entry->index == param->parameter_area_index) {
            ...
            /* If a confidential guest support object is provided then use it to set the guest state. */
            if (ctx->cgs) {
                result = ctx->cgs->set_guest_state(param->gpa, region,
                                                   param_entry->size,
    ...
    QTAILQ_FOREACH(param_entry, &ctx->parameter_data, next)
    {
        if (param_entry->index == param->parameter_area_index) {
            ...
            while (retval == 0) {
                ...
                retval =
                    ctx->cgs->get_mem_map_entry(++entry, &cgmm_entry, errp);
            }
            if (retval < 0) {
                return retval;
            }
                                     ...
                                     Error **errp)
{
    ...
    if (mem->compatibility_mask & ctx->compatibility_mask) {
        ...
        if (ctx->cgs) {
            result = ctx->cgs->set_guest_state(mem->gpa, region,
                                               mem->number_of_bytes,
...
static uint32_t supported_platform_compat_mask(QemuIgvm *ctx, Error **errp)
{
    ...
    compatibility_mask = (compatibility_mask_sev != 0) ?
                             compatibility_mask_sev :
                             compatibility_mask;
                      ...
                      Error **errp)
{
     ...
     * IGVM directives.
     */
    ctx.cgs = cgs ? CONFIDENTIAL_GUEST_SUPPORT_GET_CLASS(cgs) : NULL;

    /*
     * Check that the IGVM file provides configuration for the current
     * platform
     */
    ctx.compatibility_mask = supported_platform_compat_mask(&ctx, errp);
